...
import { convertOrderedLists } from '../../../src/helpers/convert-ordered-list.mts';
...
describe('convertOrderedLists', () => {
  it('should parse out the li element children from the node using the convertList helper', async () => {
    ...
    await convertOrderedLists(node.childNodes[0] as ElementNode);
  ...
  it('should convert the list elements using enumerate environment', async () => {
    ...
    const output = await convertOrderedLists(node.childNodes[0] as ElementNode);
